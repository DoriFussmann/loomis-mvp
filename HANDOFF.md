...
| PATCH | `/api/gap-quote/runs/[id]` | Benefits/Admin | Save Edit Proposal / entity-name edits on a persisted run |
...
4. Results and proposal Monthly Rates show one consolidated rate per tier (base rate + admin fee already summed). A footnote reads: "The rates above include an administrative fee." Un-summed base rates stay internal (admin rate table). Edit Proposal edits consolidated tier rates only. Download a 5-page branded PDF per group.

Census parsing keys off `Rel Code` when present (`SB` subscriber, `SP` spouse, `DE` dependent) and also accepts longer aliases (`Employee`, `Subscriber`, etc.) because broker formats vary. Subscriber tier counts prefer the `Tier Coverage` column on subscriber rows rather than reconstructing family composition. Entity names are proposed from filename, census employer/group fields, and email candidates. Distinctive filename keywords (e.g. "Recycling" in both `PJ Greco Census Recycling 7.16.26.xlsx` and "Greco Recycling Services, Inc.") are a strong unique match. A candidate name is assigned at most once per batch. When N files and N candidates and keyword matching locks N−1, the last file is assigned the last name by elimination. Names that resolve this way are auto-confirmed so Edit Proposal / PDF are enabled immediately. The picker is only required for genuine ambiguity (no keyword signal, or two files matching the same candidate equally); choosing a name in the dropdown confirms it — no extra Confirm click. Results include an **Edit Proposal** modal for inline edits. Inbound result pages show the request subject only (not the sender email).

Inbound runs persist to `gap_quote_runs`. A green **Save** on the results card and in Edit Proposal writes the current group (entity name + proposal fields) via `PATCH /api/gap-quote/runs/[id]`. Edits after the last save set a dirty flag and show "You have unsaved changes". **Download Proposal PDF** stays disabled while dirty, with a "Save first" hint, so the PDF always matches what is stored. The initial inbound analysis is already saved, so download is available until the user edits.

The manual paste/upload path remains ephemeral (React state only). Persist Save and PDF gating are hidden there — modal Save still applies edits in-session and PDF download stays available without a database row.
...
**After a qualifying email:** run the same `analyzeGapQuote` pipeline (including groups that fail checks), persist the run, and reply to the sender with a short message plus `https://www.epicaiproducts.com/gap-quote/results/[runId]`. Pipeline errors are surfaced in the reply rather than dropped. The results page reuses `GapQuoteClient` in stored-run mode (Edit Proposal + persist Save + PDF). Edits PATCH `/api/gap-quote/runs/[id]` on Save. Access is the existing Benefits/admin login; the link deep-links after `/login?next=…`. No token-based public access.
