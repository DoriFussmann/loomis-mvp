...
**After a qualifying email:** run the same `analyzeGapQuote` pipeline (including groups that fail checks), persist the run, and reply to the sender with a short message plus `https://www.epicaiproducts.com/gap-quote/results/[runId]`. Pipeline errors are surfaced in the reply rather than dropped. The results page reuses `GapQuoteClient` in stored-run mode (Edit Proposal + PDF). Edits PATCH `/api/gap-quote/runs/[id]`. Access is the existing Benefits/admin login; the link deep-links after `/login?next=…`. No token-based public access.
